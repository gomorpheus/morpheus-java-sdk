 ...
 * @author William Chu
 */
class DeleteServerRequestSpec extends Specification {
	...
	void "it should successfully delete a server"() {
		...
		def provisionServerRequest = new ProvisionServerRequest()
		provisionServerRequest.setServer(server)
		provisionServerRequest.getNetwork().put("name", "eth0")
		ProvisionServerResponse provisionServerResponse = client.provisionServer(provisionServerRequest)
		assert provisionServerResponse.errors.size() == 0

		DeleteServerRequest request = new DeleteServerRequest()
		request.serverId(provisionServerResponse.server.id)
